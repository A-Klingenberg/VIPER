...
def getDSSP(pdb: Union[str, Path], model: int = 0, dssp_name: str = "mkdssp") -> Bio.PDB.DSSP:
    """
    Gets the DSSP object as generated by BioPython for the given PDB, if possible.

    :param pdb: Path to the PDB file to get the DSSP tuple for
    :param model: Which model to use in the structure
    :param dssp_name: Which name the DSSP program has on your machine
    :return: The BioPython DSSP object
    """
    try:
        pdb = Path(pdb).resolve(strict=True)
    except (FileNotFoundError, RuntimeError) as e:
    ...
    parser = Bio.PDB.PDBParser(QUIET=cm().get("verbose"))
    struc = parser.get_structure("struc", pdb)[model]
    return Bio.PDB.DSSP(struc, pdb, dssp=dssp_name)
